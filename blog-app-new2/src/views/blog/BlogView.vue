    ...
    <el-container class="me-view-container">
      ...
      <div class="care">
        <el-button
          ...
          >关注</el-button
        >
        <el-button v-else icon="el-icon-remove-outline" @click="cancelCare()"
          >取消关注</el-button
        >
      </div>
    </el-container>
...
export default {
  ...
  methods: {
    ...
    isCare() {
      api({
        url: `/follow/isFollow/${this.article.id}`,
        method: "post"
      }).then(res => {
        if (res.code === 200) {
          this.isCareType = true;
        }
        if (res.code === 50001) {
          this.isCareType = false;
        }

        console.log("判断是否关注", res);
      });
    },
    // 关注
    careFuc() {
      api({
        url: `/follow/attentionTo/${this.article.id}`,
        method: "post"
      }).then(res => {
        if (res && res.code == 200) {
          this.$message({
    ...
    cancelCare() {
      api({
        url: `/follow/notAttentionTo/${this.article.id}`,
        method: "delete"
      }).then(res => {
