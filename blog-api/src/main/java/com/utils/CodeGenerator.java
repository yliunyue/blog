        ...
        FastAutoGenerator.create("jdbc:mysql://localhost:3306/blog?useUnicode=true&characterEncoding=utf-8&autoReconnect=true&rewriteBatchedStatement=true&serverTimezone=GMT",
                ...
                .globalConfig(builder -> {
                    builder.author("dragon") // 设置作者，可以写自己名字
                            ...
                            .outputDir("E:\\ider-workspace\\blog-parent\\blog-api\\src\\main\\java"); // 指定输出目录，一般指定到java目录
                })
                //包配置(PackageConfig)
                .packageConfig(builder -> {
                    builder.parent("com") // 设置父包名
                            .moduleName("") // 设置父包模块名，这里一般不设置
                            .pathInfo(Collections.singletonMap(OutputFile.xml, "E:\\ider-workspace\\blog-parent\\blog-api\\src\\main\\resources\\com\\mapper")); // 设置mapperXml生成路径，这里是Mapper配置文件的路径，建议使用绝对路径
                })
                //策略配置(StrategyConfig)
                .strategyConfig(builder -> {
                    ...
                    builder.addInclude("ms_illegal_word") // 设置需要生成的表名
